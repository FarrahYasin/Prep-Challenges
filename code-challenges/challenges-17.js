...
let arrayy = [int1];
let recur = recursionPattern(int1 - int2, int2)
return arrayy.concat(recur, arrayy[0])
...
const filterLinks = (str) => {
    // write your code here
    const reg =/www.+\w+.+(net|com|org)/g;
    const regStr = str.match(reg);
    const result = regStr.join("");
    return result;
}
// -------------------------------------------------------------------------------------------------------
...
const isPalindrome = (str) => {
    ...
    for (let i=0 ; i< Math.floor(reg.length /2) ; i++){
        if (reg[i]!=reg[reg.length-i-1]) {
            return  false;
            }       
         }
  return true;
